...
|===
|Имя |Описание
|`xref:NavFolderInfo.Flags_PR.adoc[Flags]` |Задает или возвращает флаги, определяющие местоположение папки в {wincl}е.
|`xref:NavFolderInfo.FolderId_PR.adoc[FolderId]` |Задает или возвращает идентификатор папки, предназначенной для отображения в {wincl}е.
