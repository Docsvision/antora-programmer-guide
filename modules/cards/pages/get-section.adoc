...
[NOTE]
====
Полный список методов и свойств класса секции см. в описании типа `xref:Platform-ObjectManager:SectionData_CL.adoc[SectionData]`.
====
...
В классе объектной модели карточки типа _Документ_ -- `xref:BackOffice-ObjectModel:Document_CL.adoc[Document]` -- объявлено свойство `xref:BackOffice-ObjectModel:Document.Files_PR.adoc[Document.Files]`, возвращающее коллекцию файлов документа (типа `xref:BackOffice-ObjectModel:DocumentFile_CL.adoc[DocumentFile]`), из коллекции файлов может быть получено название.
...
. Получить строки этой секции с помощью метода `xref:BackOffice-ObjectModel:BaseCard.GetSection_MT.adoc[BaseCard.GetSection]`:
+
[source,csharp]
----
var files = (IEnumerable<BaseCardSectionRow>)card.GetSection(new Guid("A6FA8BAF-2EA4-4071-AA3E-5C4E71646A90"));
...
[NOTE]
====
Чтобы получить родительский объект, например документ типа `xref:BackOffice-ObjectModel:Document_CL.adoc[Document]`, если есть объектная модель файла этого документа типа `xref:BackOffice-ObjectModel:DocumentFile_CL.adoc[DocumentFile]`, используйте метод `xref:Platform-ObjectModel:ObjectHelper.GetParent_MT.adoc[ObjectHelper.GetParent]`:
