...
----
+
* Получить все строки без учёта иерархии, т.е. для иерархической секции будут возвращены все строки:
+
[source,csharp]
----
...
<.> Перебираем строки первого уровня. Коллекцию строк без учёта иерархии можно получить из свойства `AllChildRows`.
