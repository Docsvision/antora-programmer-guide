...
Также для компиляции следует подключить дополнительные сборки из папки `C:\Program Files (x86)\Docsvision\Edi\` на Windows-машине с серверными компонентами {dv}:
