...
Сервис загрузки входящих и исходящих документов. Не рекомендуется использовать напрямую доступен, предпочтительнее вызывать методы через `xref:Edi:api/BoxesService.adoc[BoxesService]`.
