...
Поскольку базовым классом строк секций был выбран класс `xref:BackOffice-ObjectModel:BaseCardSectionRow_CL.adoc[BaseCardSectionRow]`,  преобразователь данных должен наследоваться от класса `BaseCardSectionRowMapper<T>`.
