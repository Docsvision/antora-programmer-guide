...
Расширение Консоли настройки, разрабатываемое для _Модуля учёта сетевых устройств_, должно регистрировать клинский пакет установки, устанавливать новую библиотеку карточек, а также загружать в базу данных {dv} заранее подготовленные данные карточек и справочников. Для этого в классе _SnapIn_ должно быть реализовано несколько интерфейсов.
