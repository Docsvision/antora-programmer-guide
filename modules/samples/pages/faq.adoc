...
----
+
Для получения сертификата сотрудника с использованием метода `GetCertificate`, сертификат должен быть указан в соответствующем поле (см. Руководство по настройке), в xref:dev@backoffice:desdirs:staff/employees/main-tab.adoc#certificate[Справочнике сотрудников].
+
[[get-certificate]]
