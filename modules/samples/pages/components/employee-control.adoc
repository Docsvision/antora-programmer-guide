...
+
* Полный путь к сборке -- если сборка не зарегистрированаfootnote:[Для регистрации сборку необходимо поместить в одну из папок на сервере с установленной программой {cns} (см. раздел "xref:install-linux::linux-limitations.adoc[]"), перечисленных в ветке реестра `{hklm-dv}\Shared\Paths`.]. Например: `C:\Users\KurkinSA\AppData\Local\Docsvision\Client\Docsvision.TreeControl.dll`.
* Строгое имя сборки -- если сборка зарегистрированаfootnote:[Для регистрации сборку необходимо поместить в одну из папок на сервере с установленной программой {cns} (см. раздел "xref:install-linux::linux-limitations.adoc[]"), перечисленных в ветке реестра `{hklm-dv}\Shared\Paths`.]. Например: `Docsvision.TreeControl, Version=1.0.0.0, Culture=neutral, PublicKeyToken=7148afe997f90519`.
