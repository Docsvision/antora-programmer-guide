...
[NOTE]
====
Если для реализации пользовательского интерфейса используются компоненты WinForms,
// (или DevExpress),
а не WPF, то нужно наследовать свой класс от типа `DocsVision.Platform.WinForms`, реализованного в сборке `DocsVision.Platform.WinForms.dll`. +
