:page-aliases: BackOffice-ObjectModel-Services-IPartnersService:IPowerOfAttorneyService_IN.adoc
...
Func<string, ObjectContext, PowersCode>,
PowerOfAttorneyHandlingFlags);`
a|Импортирует СКД представителя -- юридического лица из файла МЧД
...
Func<string, ObjectContext, PowersCode>,
PowerOfAttorneyHandlingFlags);`
a|Импортирует СКД представителя -- юридического лица из файла МЧД, выданной в порядке передоверия
