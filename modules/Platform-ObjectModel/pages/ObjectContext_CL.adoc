...
|`xref:ObjectContext.UserAccount_PR.adoc[UserAccount]` |Возвращает название учётной записи текущего пользователя.
|===

WARNING: Объектная модель семейства `ObjectContext` потоконебезопасна (не thread-safe), поэтому важно использовать межпоточную синхронизацию или не использовать её параллельно.
