...
В приведенном примере выполняется поиск сотрудника с названием учётной записи "DOMAIN\\IvanovII"
...
Если сотрудника с учётной записью `DOMAIN\IvanovII` нет в справочнике, поиск вернет `null`. Можно проверить, добавив скрипт на кнопку:
