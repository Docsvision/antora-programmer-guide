...
.. Нажмите menu:File[Open] и выберите файл `{wcd}\Site\Content\Tools\DVWebTool\Application Files\DocsVision.DVWebTool.exe.manifest`.
...
.. Нажмите menu:File[Open] и выберите файл `{wcd}\Site\Content\Tools\DVWebTool\DocsVision.DVWebTool.application`.
.. Перейдите в раздел _Name_ и в поле `Version` увеличьте номер сборки. Например, `5.5.5531.0` до `5.5.5531.1`.
+
WARNING: Не изменяйте мажорную и минорную версии, и версию исправления.
+
...
.. Перейдите в раздел _Application Reference_, нажмите кнопку *Select Manifest* и выберите файл `{wcd}\Content\Tools\DVWebTool\Application Files\DocsVision.DVWebTool.exe.manifest`. В поле Version будет указана версия, полученная при обновлении файла манифеста.
