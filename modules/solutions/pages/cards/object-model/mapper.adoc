...
Преобразователь данных для карточки оформляется аналогичным образом, за исключением базового класса (должен быть класс `xref:BackOffice-ObjectModel:Mapping/BaseCardMapper_CL.adoc[BaseCardMapper<T>]`) и того, что в карте присутствуют исключительно секции.
