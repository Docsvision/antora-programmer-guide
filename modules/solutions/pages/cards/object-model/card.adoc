...
В библиотеке карточек _{bo}_ базовым классом карточки является класс `xref:BackOffice-ObjectModel:BaseCard_CL.adoc[BaseCard]`, дополняющий функциональность класса `ObjectBase`. Базовый класс также можно использовать в качестве базового для объектной модели свой карточки. Для справочников в данном случае базовым классом выступает `xref:BackOffice-ObjectModel:BaseDictionaryCard_CL.adoc[BaseDictionaryCard]`.
