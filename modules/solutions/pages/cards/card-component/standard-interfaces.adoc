...
Сначала необходимо создать новый проект -- компонент, который будет реализовывать визуальную часть карточки. Расширения {wincl}а также являются специальными компонентами карточек.
