...
|`GetDuration(Guid, DateTime, DateTime)` |Рассчитывает длительность (в часах) заданного периода времени с учётом плана в бизнес-календаре.
|`GetEndDate(Guid, DateTime, Double)` |Рассчитывает конечное время с учётом плана в бизнес-календаре и указанной длительности.
|`GetStartDate(Guid, DateTime, Double)` |Рассчитывает начальное время с учётом плана в бизнес-календаре и указанной длительности.
|===
