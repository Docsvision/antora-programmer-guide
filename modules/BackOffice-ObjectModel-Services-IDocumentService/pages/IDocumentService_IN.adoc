...
|`xref:BackOffice-ObjectModel-Services-IDocumentService:IDocumentService.CreateDocument_3_MT.adoc[CreateDocument(String, KindsCardKind, DocumentVersioningType)]` |Создаёт документ с заданным видом карточки и добавляет основной файл с указанным типов версионинга.
