...
Чтобы контекст объектов мог работать с типами карточек и секций, для которых разработаны _фабрики преобразователей данных_, эти фабрики должны быть добавлены в контекст (см. на примере добавления фабрики `BackOfficeMapperFactory` в xref:DM_FullContextInit.adoc[примере]).
