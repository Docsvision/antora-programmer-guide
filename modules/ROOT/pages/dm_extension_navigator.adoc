...
[NOTE]
====
[.note__title]#Важное замечание:# В схеме карточки расширения должен быть установлен флажок [.keyword]*"Реализует расширения интерфейса хоста" / "Provides UI Extension"*. Также рекомендуется установить флажок [.keyword]*"Является справочником" / "Dictionary (only one instance allowed)"*, чтобы исключить возможность создания нескольких экземпляров карточки-расширения, что может привести к конфликтам при их вызове.
====
...
image::img/dm_extension_navigator.png[image]
...
Тип link:../api/DocsVision/Platform/Extensibility/NavExtensionTypes_EN.adoc[NavExtensionTypes.FolderTree].
