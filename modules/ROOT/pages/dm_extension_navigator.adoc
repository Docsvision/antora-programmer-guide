...
Технически расширение {wincl}а представляет собой обычную карточку системы {dv}, которая должна разрабатывается по стандартному xref:dm_developmentcards.adoc[сценарию].
