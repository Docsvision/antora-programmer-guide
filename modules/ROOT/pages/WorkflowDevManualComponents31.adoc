...
Вариант с xref:..xref:api/DocsVision/Workflow/Functions/ExecResultEnum_EN.adoc[ExecResultEnum] позволяет после завершения сценария оставить функцию активной или ожидающей какого-то события - сценарий будет запущен повторно после наступления события или (в зависимости от [.keyword .apiname]#ExecResultEnum#) при следующем проходе.
...
Кроме того, предусмотрено использование переменной типа xref:..xref:api/DocsVision/Workflow/Runtime/ProcessVariable_CL.adoc[ProcessVariable] - переменная процесса. В этом случае алгоритм получения значения переменной следующий:
