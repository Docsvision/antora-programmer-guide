...
Практически каждый метод веб-сервиса требует идентификатор пользовательской сессии, поэтому первоначальной задачей является создание сессии пользователя. Для этого предназначен метод xref:appendix/webservice/server/SessionLogin.adoc[SessionLogin]. Код для открытия пользовательской сессии может выглядеть следующим образом:
...
. Создать объект-файл в базе данных, используя метод xref:appendix/DevManualAppendix_WebService_Files_FileCreate.adoc[FileCreate].
. Открыть созданный объект для записи, используя метод xref:appendix/DevManualAppendix_WebService_Files_FileOpen.adoc[FileOpen].
. Записать содержимое файла с файловой системы в открытый объект-файл, используя метод xref:appendix/DevManualAppendix_WebService_Files_FileWrite.adoc[FileWrite].
. Закрыть с сохранением открытый объект, используя метод xref:appendix/DevManualAppendix_WebService_Files_FileClose.adoc[FileClose]
