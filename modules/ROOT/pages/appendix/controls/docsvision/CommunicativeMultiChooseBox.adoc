...
* image:buttons/phone-state.png[Телефонная трубка] -- флаг состояния выбранного сотрудника в Microsoft Lynс Server. Открывает окно отправки сообщения сотрудникам.
* image:buttons/triangle-down.png[Треугольник вниз] -- открывает окно выбора сотрудников.
