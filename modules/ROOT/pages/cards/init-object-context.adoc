...
После того как было выполнено подключение к серверу {dv} -- xref:cards/connect-to-dv-server.adoc[создана пользовательская сессия] -- может быть создан xref:cards/general-information.adoc#session-context[контекст объектов] для работы с методами API уровня бизнес-логики.
...
. Используя платформенные сервисы, загрузить в контекст объектов xref:cards/general-information.adoc#mappers[преобразователи данных] и xref:cards/general-information.adoc#services[необходимые сервисы].
