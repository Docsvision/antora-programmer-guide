...
image::img/dev_card_21.png[[.fig--title-label]##Рис. 1. ##Создание проекта «Windows Forms Control Library»,width=500]
...
[NOTE]
====
[.note__title]#Прим.:# Эти сборки входят в состав как серверной, так и клиентской программ инсталляции. В случае установки только клиентской инсталляции, их можно взять из папки пользователя, для которого проведена установка: [.ph .filepath]``SYSTEMDISK`:\Users\`USERNAME`\AppData\Local\DocsVision\5.5\Client`

При выполнении установки сервера сборки можно взять из папки: [.ph .filepath]``SYSTEMDISK`:\Program Files\DocsVision\Platform\5.0`
====

Следует подключить пространства имен этих сборок в область видимости текущего проекта при помощи директивы [.keyword]*using*:
...
Шаблон класса карточки, при использовании базового класса [.keyword .apiname]#BaseCardControl# и [.dfn .term]_Конструктора разметок_, требует изменения:
...
* *xref:../pages/CardsDevCompEvent.adoc[Обработка событий]* +
* *xref:../pages/CardsDevComp_Reuse.adoc[Переиспользование компонента карточки]* +
