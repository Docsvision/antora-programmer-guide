...
----
<.> `card` -- xref:development-manual/get-card.adoc[данные карточки], из которой выбираются данные секции.
...
----
+
* Найти строку, пользуясь xref:development-manual/search.adoc#filter[клиентской фильтрацией], либо строки, используя поиск по секции.
