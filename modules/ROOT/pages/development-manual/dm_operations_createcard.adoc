...
* userSession -- xref:development-manual/dm_connection.adoc[пользовательская сессия];
...
API данного уровня не предлагает отдельного метода для создания нового экземпляра карточки. Данная задача распределена между преобразователем данных соответствующего типа карточки и сервисом для работы с этим типом карточек (понятие преобразователя и сервиса приведено в разделе xref:development-manual/dm_connection.adoc[Подключение к серверу Docsvision]).

[NOTE]
====
[.note__title]#Прим.:# Полное описание _сервисов_, предоставляемых в рамках базовой поставки Docsvision, было приведено в разделе xref:development-manual/dm_services.adoc[Сервисы объектной модели], также можно обращаться к описанию интерфейсов сервисов в разделе xref:api/Intro.adoc[Библиотека классов].
====
...
* objectContext -- xref:development-manual/dm_connection.adoc[контекст объектов];
