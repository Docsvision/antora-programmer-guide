= DocsVision.BackOffice.ObjectModel.Services.Entities - пространство имен
...
|xref:ActiveDirectory/ActiveDirectory_NS.adoc[ActiveDirectory] |В пространстве имен DocsVision.BackOffice.ObjectModel.Services.Entities.ActiveDirectory представлены типы доступные для синхронизации с доменом ActiveDirectory.
|xref:KindSetting/KindSetting_NS.adoc[KindSetting] |Пространство имен DocsVision.BackOffice.ObjectModel.Services.Entities.KindSetting содержит типы, определяющие параметры вида карточки, определенные в справочнике видов карточек.
|===
...
|===
|Класс |Описание
|xref:BaseCardSignatureVerification_CL.adoc[BaseCardSignatureVerification] |Возвращает информацию по проверке подписи на карточке.
...
|xref:CheckSignatureResult_CL.adoc[CheckSignatureResult] |Представляет результат проверки подписи.
...
|xref:MailNotificationInfo_CL.adoc[MailNotificationInfo] |Предоставляет данные электронного сообщения для сервиса отправки электронной почты.
...
|xref:PerformerSearchWordsManager_CL.adoc[PerformerSearchWordsManager] |Представляет статические методы работы с поисковыми словами.
...
|xref:StartBusinessProcessErrorInfo_CL.adoc[StartBusinessProcessErrorInfo] |Представляет содержимое ошибки запуска бизнес-процесса.
|xref:TaskStopExecutionInfo_CL.adoc[TaskStopExecutionInfo] |Представляет содержимое ошибки остановки исполнения задания.
|xref:TaskCopyResultsOptions_CL.adoc[TaskCopyResultsOptions] |Предоставляет параметры переноса отчета об исполнении подчиненного задания в родительское.
...
|ViewCardField |Определяет свойства поля для [.keyword]*представления*.
|ViewCardFieldsGroup |Определяет свойства группы полей [.keyword]*представления*.
|===
...
|xref:ILongProcessManager_IN.adoc[ILongProcessManager] |Добавляется в производный класс возможности менеджера длительных процессов.
...
|xref:BusinessProcessErrorType_EN.adoc[BusinessProcessErrorType] |Определяет типы ошибок формируемых при проверке бизнес-процесса.
|xref:DocumentPropertyDirection_EN.adoc[DocumentPropertyDirection] |Определяет направление синхронизации свойств документа и полей карточки.
|xref:GridViewFieldCollectionType_EN.adoc[GridViewFieldCollectionType] |Определяет тип отображаемого поля справочника сотрудников или контрагентов.
|xref:PerformerType_EN.adoc[PerformerType] |Определяет тип исполнителя задания.
|xref:TaskTreeNodeType_EN.adoc[TaskTreeNodeType] |Определяет тип узла в дереве заданий.
|===
