...
|===
|Имя |Описание
|xref:DVGate.BaseURL_PR.adoc[BaseURL] |Возвращает URL к корню WEB-сервера Docsvision.
...
|xref:DVGate.FoldersCard_PR.adoc[FoldersCard] |Возвращает карточку папок.
|xref:IGate.ID_PR.adoc[ID] |Возвращает уникальный идентификатор шлюза.
|xref:DVGate.MessageChannel_PR.adoc[MessageChannel] |Возвращает канал сообщений шлюза.
|xref:DVGate.RefPartners_PR.adoc[RefPartners] |Возвращает справочник контрагентов.
|xref:DVGate.RefStaff_PR.adoc[RefStaff] |Возвращает справочник сотрудников.
|xref:DVGate.RefUniversal_PR.adoc[RefUniversal] |Возвращает универсальный справочников (Делопроизводство).
...
|===
|Имя |Описание
|xref:DVGate.AddVariable_MT.adoc[AddVariable(Int32, String)] |Добавляет новую переменную шлюза.
|AddVariable(Int64, String) |Добавляет новую переменную шлюза.
|xref:IGate.CompareVariables_MT.adoc[CompareVariables(Int32, String, String)] |Сравнивает значения двух однотипных переменных шлюза. Выполняется простое сравнение двух строковых переменных.
...
|xref:DVGate.CopyCard_MT.adoc[CopyCard(DVCard, Boolean)] |Создает копию переданной карточки.
|xref:DVGate.CopyCards_MT.adoc[CopyCards(Array, Boolean, ProcessVariable)] |Создает копии нескольких карточек.
|xref:DVGate.CopyVariable_MT.adoc[CopyVariable(Int32, String, String)] |Создает копию переменной шлюза.
...
|xref:DVGate.CreateCard_MT.adoc[CreateCard(String)] |Создает новую карточку указанного типа.
|CreateCardInFolder(DVCardType, DVFolder) |Создает новую карточку указанного типа и добавляет ярлык для её открытия в папку.
|xref:DVGate.CreateCardInFolder_MT.adoc[CreateCardInFolder(String, DVFolder)] |Создает новую карточку указанного типа и ярлык на неё.
|xref:DVGate.CreateDVFileFromExAttachment_MT.adoc[CreateDVFileFromExAttachment(ExAttachment)] |Создает карточку версионного файла на основе вложения письма переданного шлюзом к Exchange.
|xref:DVGate.CreateDVFileFromExAttachmentInFolder_MT.adoc[CreateDVFileFromExAttachmentInFolder(ExAttachment, DVFolder, ProcessVariable)] |Создает карточку версионного файла на основе вложения письма переданного шлюзом к Exchange.
|xref:DVGate.CreateDVFileFromFSFile_MT.adoc[CreateDVFileFromFSFile(FSFile)] |Создает карточку версионного файла на основе файла, полученного из шлюза к файловой системе.
|xref:DVGate.CreateDVFileFromFSFileInFolder_MT.adoc[CreateDVFileFromFSFileInFolder(FSFile, DVFolder, ProcessVariable)] |Создает карточку версионного файла на основе файла, полученного из шлюза к файловой системе, и ярлык на карточку.
|xref:DVGate.CreateFolder_MT.adoc[CreateFolder(DVFolder, String, DVCardRow)] |Создает новую папку указанного типа.
...
|xref:DVGate.DeleteCard_MT.adoc[DeleteCard(DVCard, Boolean)] |Удаляет карточку.
|xref:DVGate.DeleteFolder_MT.adoc[DeleteFolder(DVFolder, Boolean)] |Удаляет папку.
|xref:DVGate.DeleteShortcut_MT.adoc[DeleteShortcut(DVShortcut, Boolean)] |Удаляет ярлык на карточку.
|xref:DVGate.DeleteVariable_MT.adoc[DeleteVariable(Int32, String)] |Удаляет переменную шлюза.
|xref:DVGate.GetCard_MT.adoc[GetCard(Guid, Boolean)] |Получает карточку.
|xref:DVGate.GetCardHardLink_MT.adoc[GetCardHardLink(String)] |Получает сильный ярлык на карточку.
|xref:DVGate.GetCardSecurable_MT.adoc[GetCardSecurable(Guid)] |Возвращает описатель прав для карточки.
|xref:DVGate.GetCardTypeByID_MT.adoc[GetCardTypeByID(Guid)] |Получает тип карточки по его идентификатору.
|xref:DVGate.GetCurrentProcess_MT.adoc[GetCurrentProcess(ProcessVariable)] |Получает текущий бизнес-процесс.
|xref:DVGate.GetDocStateByName_MT.adoc[GetDocStateByName(DVCardRow, String)] |Получает строку состояния карточки по названию.
|xref:DVGate.GetDocStates_MT.adoc[GetDocStates(DVCardRow, ProcessVariable)] |Получает все возможные состояния карточки и сохраняет их в переменную процесса.
|xref:DVGate.GetDocumentByTypeID_MT.adoc[GetDocumentByTypeID(String)] |Получает справочник по его идентификатору.
|xref:DVGate.GetFunctionLinkedCard_MT.adoc[GetFunctionLinkedCard(String, Boolean, ProcessVariable)] |Получает карточку, связанную с функцией бизнес-процесса.
|xref:DVGate.GetRow_MT.adoc[GetRow(Guid, Guid)] |Получает строку секции карточки.
|xref:DVGate.GetVariable_MT.adoc[GetVariable(Int32, String, Object)] |Получает переменную процесса по её идентификатору и типу.
|xref:DVGate.GetVariableReference_MT.adoc[GetVariableReference(Int32, String)] |Возвращает URL (ссылку) для открытия карточки или папки.
|xref:DVGate.ImportCard_MT.adoc[ImportCard(String)] |Импортирует карточку из XML.
|xref:DVGate.ImportCardFromFile_MT.adoc[ImportCardFromFile(FSFile)] |Импортирует карточку из текстового файла.
|xref:DVGate.ImportCardIntoFolder_MT.adoc[ImportCardIntoFolder(String, DVFolder, ProcessVariable)] |Импортирует карточку из XML и создаёт ярлык.
|xref:DVGate.ImportCardIntoFolderFromFile_MT.adoc[ImportCardIntoFolderFromFile(FSFile, DVFolder, ProcessVariable)] |Импортирует карточку из текстового файла и создаёт ярлык.
|Initialize(Guid, IMessageChannel, ISubscriptionChannel) |Инициализирует новый экземпляр шлюза.
|xref:DVGate.IsVariableExists_MT.adoc[IsVariableExists(DVVariableType, String)] |Проверяет существование переменной шлюза
|xref:DVGate.SendMessageToProcess_MT.adoc[SendMessageToProcess(Guid, Guid, Int32, String, Guid)] |Отправляет сообщение функции процесса.
|xref:DVGate.SendMessageToProcessFunctions_MT.adoc[SendMessageToProcessFunctions(Guid, String, Int32, String, Guid)] |Отправляет сообщение функции процесса.
|===
...
|===
|Имя |Описание
|xref:DVGate.GateID_FL.adoc[GateID] |Предоставляет идентификатор шлюза.
|xref:DVGate.GateName_FL.adoc[GateName] |Предоставляет название шлюза.
|===
