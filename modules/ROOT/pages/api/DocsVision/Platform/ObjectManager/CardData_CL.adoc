...
Содержит данные карточки. Этот класс является [.keyword]*абстрактным*.
...
|xref:CardData.CancelUpdate_MT.adoc[CancelUpdate] |Отменить несохраненные изменения и выключить режим отложенных изменений.
