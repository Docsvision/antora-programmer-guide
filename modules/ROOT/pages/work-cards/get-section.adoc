...
----
<.> `card` -- xref:work-cards/get-card.adoc[данные карточки], из которой выбираются данные секции.
...
----
+
* Найти строку, пользуясь xref:work-cards/search.adoc#filter[клиентской фильтрацией], либо строки, используя поиск по секции.
