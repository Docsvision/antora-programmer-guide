...
----
* Метод QueryInterface(SnapInInterfacesEnum)# -- возвращает пользовательский интерфейс, если он предусмотрен, для различных режимов работы _Консоли настройки_. Подробнее см. в разделе xref:DM_ConsolePlugin.adoc[Модуль расширения Консоли настройки].
+
[source,csharp]
----
