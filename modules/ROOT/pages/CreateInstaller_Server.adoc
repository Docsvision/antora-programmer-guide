...
[NOTE]
====
[.note__title]#Прим.:# Исходный код _SnapIn_ расположен в подкаталоге `NetstatSolutionInstaller` архива проекта.
====
...
. Создать подкаталог _решения_ в папке Docsvision (по умолчанию `C:\Program Files (x86)\Docsvision`).
...
----
. Вызвать Консоль настройки Docsvision с параметрами: `"C:\Program Files (x86)\Docsvision\Platform\5.5\Tools\ServerConsole.exe" /c /n \{Название решения}`. В проекте пакета установки серверной части за это отвечает сценарий:
+
...
При удалении решения, необходимо вызвать _Консоль настройки_ Docsvision с параметрами: `"C:\Program Files (x86)\Docsvision\Platform\5.5\Tools\ServerConsole.exe" /rs /n \{Название решения}`. В проекте пакета установки серверной части за это отвечает сценарий:
