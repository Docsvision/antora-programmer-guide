...
[NOTE]
====
[.note__title]#Прим.:# Для класса, унаследованного от xref:api/DocsVision/BackOffice/WinForms/ScriptClassBase_CL.adoc[DocsVision.BackOffice.WinForms.ScriptClassBase], _контекст объектов_ будет доступен через свойство `base.CardControl.ObjectContext`.
====
