...
В примере, мы регистрируем только одну команду, которая вызывает метод xref:api/DocsVision/Platform/CardHost/ICardHost.ActivateFolder_MT.adoc[`ActivateFolder(Guid, Boolean)`], выполняющий позиционирование курсора на определённой папке {wincl}а. Папка может быть обычной, либо виртуальной.
