...
. Скопируйте `safeprojectnamesafeprojectname.dll` и `safeprojectnamesafeprojectname.dll.pdb` из папки `bin\Plugins` в `Каталог-установки-{wc}а\Plugins`.
+
Ресурсные сборки скопируйте в папки `{wcd-win}\ru\` (для русской локализации), `{wcd-win}\uk\` (для английской локализации) и т.д.
+
. Перезапустите {kvr}.
