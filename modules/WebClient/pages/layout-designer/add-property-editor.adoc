...
----
+
Собственная реализация редактора может включать любые элементы управления и использоваться для отображения любых значений (с учётом рекомендуемых значений элемента управления).
+
...
----
// +
// . Скомпилируйте проект и скопируйте полученную сборку на сервер {wc}а в папку `{wcd-lin}/Plugins/%Каталог Решения%`. Ресурсные сборки скопируйте в папки `{wcd-lin}/ru/` (для русской локализации), `{wcd-lin}/uk/` (для английской локализации) и т.д.
// +
// . Перезапустите {kvr}.
