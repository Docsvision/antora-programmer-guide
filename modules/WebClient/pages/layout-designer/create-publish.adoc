...
[WARNING]
====
include::5.5.17@webclient:ROOT:partial$excerpts.adoc[tags=versions]
====
+
...
****
+
. Скопируйте файл расширения на сервер {wc}а в папку `{wcd-win}\Plugins\%Каталог Решения%`. Каталог Решения необходимо создать самостоятельно. Ресурсные сборки нужно скопировать в папки `{wcd-win}\ru\` (для русской локализации), `{wcd-win}\uk\` (для английской локализации) и т.д.
+
. Перезапустите {kvr}.