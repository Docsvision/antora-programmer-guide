...
// ****
// +
// . Скопируйте файл расширения на сервер {wc}а в папку `{wcd-lin}/Plugins/%Каталог Решения%`. Каталог Решения необходимо создать самостоятельно. Ресурсные сборки нужно скопировать в папки `{wcd-lin}/ru/` (для русской локализации), `{wcd-lin}/uk/` (для английской локализации) и т.д.
// +
// . Перезапустите {kvr}.