...
+
Подробное описание процедуры загрузки пользовательской библиотеки карточек содержится в xref:solutions:cards/scheme/load-scheme.adoc[руководстве разработчика {dv}].
+
...
. Запустите службу Сервера {wc}а из папки `SamplesOutput\ExternalWebService\WebService`.
.. На Windows запустите `WebService.exe` из под учётной записи {dv} {wc}а.
.. На Linux запустите командой `dotnet WebService.dll`
.. Для запуска под нужной учётной записью можно задать параметры `SystemUserAccount` и `SystemUserPassword` в `appsettings.json`.
