...
NOTE: Программа _DVSupService_ устанавливается на клиентскую машину только в ОС семейства Linux. Для ОС {ms} Windows используется _DVWebTool_, см. подробнее "xref:webclient:user:prepare-add-components.adoc#dvWebTool[Установка и запуск службы DVWebTool]".
...
. Скопируйте папку `SamplesOutput\Site\Content\Modules\WatermarkWebExtension\` в `{wcd-lin}/Content/Modules`.
...
+
. Скопируйте папку `SamplesOutput\Site\Extensions\WatermarkServerExtension` в `Путь к сайту {wc}а/Extensions`.
+
