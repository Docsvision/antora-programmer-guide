...
. Для работы пользователей на Windows скопируйте файлы из папки `SamplesOutput/Site/Content/Tools/DVWebTool/Application Files/` в `C:\Program Files (x86)\Docsvision\WebClientToolkit\WebToolBuilder\DVWebTool`.
+
...
. Установите собранную утилиту DvWebTool на сервер {wc}а, см. подробнее "xref:6.1@webclient:admin:webservices-install.adoc[]" в документации {wc}а.
+
include::dv6@system:ROOT:partial$restart-wcs.adoc[]
...
. Сохраните разметку.
. Установите или обновите программу _DVWebTool_. Cм. пункт xref:6.1@webclient:admin:webservices-install.adoc#wtbldr[Установка DVWebTool при помощи WebToolBuilder] в руководстве пользователя {wc}.
