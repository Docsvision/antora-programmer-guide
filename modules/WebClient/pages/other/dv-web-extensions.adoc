...
include::6.1@webclient:admin:partial$excerpts.adoc[tags=dvweb-win]
...
. Остановите службу Сервера {wc}а.
. Скопируйте папку `SamplesOutput/Content/Modules/WatermarkWebExtension/` в `{wcd-lin}/Content/Modules`.
. Скопируйте папку `SamplesOutput/Extensions/WatermarkServerExtension` в `{wcd-lin}/Extensions`.
. Для работы пользователей на Windows скопируйте файлы из папки `SamplesOutput/Site/Content/Tools/DVWebTool/Application Files/` в `C:\Program Files\Docsvision\WebClientToolKit\WebToolBuilder\DVWebTool\`.
+
...
. Установите собранную утилиту DvWebTool на сервер {wc}а, см. подробнее "xref:new-admin-console@webclient:admin:webservices-install.adoc[]" в документации {wc}а.
+
include::new-admin-console@system:ROOT:partial$restart-wcs.adoc[]
...
. Сохраните разметку.
. Установите или обновите программу _DVWebTool_. Cм. пункт xref:new-admin-console@webclient:admin:webservices-install.adoc#wtbldr[Установка DVWebTool при помощи WebToolBuilder] в руководстве пользователя {wc}.
