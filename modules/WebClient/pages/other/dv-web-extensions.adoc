...
. Для работы пользователей на Windows скопируйте файлы из папки `SamplesOutput/Site/Content/Tools/DVWebTool/Application Files/` в `Путь к папке с панелью управления {wc}ом/Site/Content/Tools/DVWebTool/`, например `C:/Program Files (x86)/Docsvision6/WebClient/Site/Content/Tools/DVWebTool`.
. Запустите *{wcs-new}*.
...
. Установите собранную утилиту DvWebTool на сервер {wc}а, см. подробнее "xref:webclient:user:prepare-add-components.adoc#dvWebTool[Установка и запуск программы DVWebTool]".
...
. Сохраните разметку.
. Установите или обновите программу _DVWebTool_. Cм. пункт xref:webclient:user:prepare-add-components.adoc#dvWebTool[Установка и запуск программы DVWebTool] в руководстве пользователя {wc}.
