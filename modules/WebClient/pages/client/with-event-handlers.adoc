...
+
. Скопируйте полученный файл `%BuildDir%/extension.js` на сервер {wc}а в папку `{wcd-lin}/Content/Modules/%Каталог Решения%`.
+
...
+
NOTE: Инструкция по работе в программе {kvr} приведена в документации по работе с программной xref:webclient:layouts:info-install.adoc[{kvr}].
+
