...
+
. Скопируйте полученный файл `%BuildDir%\extension.js` на сервер {wc}а в папку `{wcd-win}\5.5\Site\Content\Modules\%Каталог Решения%`.
+
...
+
. После завершения отладки скрипта, соберите и опубликуйте на сервере {wc}а "релизную версию" клиентского расширения. "Релизная версия" собирается командой `npm run build:prod`. Посмотреть все назначенные обработчики событий можно на вкладке _Скрипты_, подробнее см. "xref:webclient:layouts:controls-event-handler.adoc[]".
