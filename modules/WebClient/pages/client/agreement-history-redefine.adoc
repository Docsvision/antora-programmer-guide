...
Элемент управления `*Ход согласования*` xref:webclient:layouts:ctrl/approval/agreementHistory.adoc[представляет собой] таблицу, в которой отображаются события заданий согласования (выполнение, делегирование и др.).
...
+
Будет выполнена загрузка скриптов.
...
NOTE: Чтобы получить другие примеры, включая пример получения списка файлов для отображения столбца _Правки_, обратитесь к исходному коду элемента управления `_Ход согласования_` в каталоге установки {wc}а: `{wcd-lin}/Content/App/Approval/Controls/AgreementHistory`.
