...
* https://nodejs.org/en/[NodeJS {node-v} и выше]
...
+
 ...
 $ npm run build:prod
+
. Скопируйте каталог `SamplesOutput\Site\Content\Modules\FileListHighlightRow` в каталог `Путь-к-установленному-{wc}у\Site\Content\Modules`.
. Перезапустите IIS.
