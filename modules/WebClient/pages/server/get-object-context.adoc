...
WARNING: Не используйте метод `GetOrCreateApplicationPoolSessionContext`, это приведет к блокировкам потоков под нагрузкой. Вместо `GetOrCreateApplicationPoolSessionContext` рекомендуется использовать `GetOrCreateExclusivePoolSessionContext` с освобождением сессий (см. ниже).
