...
Особенности использования стандартного способа формирования описаний, а также формирования описаний с помощью разметок рассмотрены см. в описании элемента управления xref:webclient:layouts:ctrl/special/links.adoc[Ссылки] в xref:webclient:layouts:info-install.adoc[Руководстве пользователя программы {kvr}].
...
Расширение по данному примеру распространяется только на ссылки на карточки. Для URL-ссылок штатный механизм формирования описания не вызывается. URL-ссылка отображается с адресом URL, а значение поля _Описание ссылки_ не используется для формирования отображаемого текста.
