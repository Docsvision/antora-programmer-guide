...
* https://github.com/Docsvision/WebClient-Samples/tree/master/Install/WebClientSDK[{dv} WebClient SDK].
...
+
NOTE: При разработке собственных конечных точек рекомендуется всегда получать текущую сессию, т.к. при этом вызове происходит проверка доступа (в частности, проверка пользовательских cookie).
+
...
. Убедитесь, что установлен https://github.com/Docsvision/WebClient-Samples/tree/master/Install/WebClientSDK[{dv} WebClient SDK]K.
+
В этом случае в переменной окружения `DocsvisionWebClientSDK` содержится путь к его папке, например, `C:\Program Files\Docsvision\WebClient\SDK`.
+
...
+
****
include::6.1@webclient:ROOT:partial$excerpts.adoc[tags=versions]
****
+
...
****
+
. Скопируйте `safeprojectnamesafeprojectname.dll` и `safeprojectnamesafeprojectname.dll.pdb` из папки menu:bin[WebClient > Bin] в menu:Каталог-установки-{wc}а[bin].
+
[WARNING]
====
Расширения можно устанавливать в папки menu:Каталог-установки-{wc}а[Extensions] и menu:Каталог-установки-{wc}а[bin]. В ходе разработки расширения можно удобнее устанавливать в папку menu:Каталог-установки-{wc}а[bin], т.к. это избавляет от необходимости перезапускать службу Сервера {wc}а при установке сборок.

В ресурсной поставке рекомендуется устанавливать сборки в папку menu:Каталог-установки-{wc}а[Extensions], чтобы проще было видеть, какие расширения установлены в {wc}е. Однако этот способ требует перезапуска службы Сервера {wc}а при установке или обновлении сборки.

При обновлении существующих файлов достаточно обновить страницу в браузере с очисткой кэша (например kbd:[CTRL+SHIFT+R])
====
+
