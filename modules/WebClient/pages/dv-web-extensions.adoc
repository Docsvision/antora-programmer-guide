...
Для работы функции автоматического закрытия DVWebTool необходимо `WebToolExtensionManager` наследовать `IServiceManagerEx` и зарегистрировать `OpenFilesController` через `AddWebSocketService` с двумя параметрами: `AddWebSocketService<TBehavior>(string path, Func<TBehavior> creator)`. В `creator` нужно проинициализировать `OnMessageReceived` и `OnMessageProcessed`, например, так:

[source,csharp]
----
OpenFilesController Creator()
{
	OpenFilesController controller = new OpenFilesController();
	controller.OnMessageReceived = onMessageReceived;
	controller.OnMessageProcessed = onMessageProcessed;
	return controller;
}
server.AddWebSocketService("/OpenFiles", Creator);
----

Аналогичные действия справедливы, если необходимо автоматическое закрытие DVWebTool после выполнения действий из `PrintFilesController`.
...
. Остановите IIS.
. Скопируйте папку `SamplesOutput\Site\Content\Modules\WatermarkWebExtension\` в `{wcd-win}\Site\Content\Modules`.
. Скопируйте папку `SamplesOutput\Site\Content\Tools\DVWebTool\Application Files\` в `{wcd-win}\Site\Content\Tools\DVWebTool\`.
. Скопируйте папку `SamplesOutput\Site\Extensions\WatermarkServerExtension` в `{wcd-win}\Site\Extensions`.
. Запустите IIS.

[start=11]
.Регистрация расширения _DVWebTool_ на сервере {wc}.
. Запустите программу `{wcd-win}\Tools\mageui.exe`.
. Обновите манифест программы:
.. Нажмите menu:File[Open] и выберите файл `{wcd-win}\Site\Content\Tools\DVWebTool\Application Files\DocsVision.DVWebTool.exe.manifest`.
...
****
+
.. Нажмите *…* (три точки) в поле _File_, выберите файл сертификата `{wcd-win}/DVWebTool.pfx`, затем нажмите *OK* в основном окне подписания манифеста (пароль указывать не нужно). Файл манифеста будет подписан сертификатом `DVWebTool.pfx`.

. Обновите файл развертывания программы:
.. Нажмите menu:File[Open] и выберите файл `{wcd-win}\Site\Content\Tools\DVWebTool\DocsVision.DVWebTool.application`.
.. Перейдите в раздел _Name_ и в поле `Version` увеличьте номер сборки. Например, `5.5.5531.0` до `5.5.5531.1`.
+
WARNING: Не изменяйте мажорную и минорную версии, и версию исправления.
+
...
.. Перейдите в раздел _Application Reference_, нажмите кнопку *Select Manifest* и выберите файл `{wcd-win}\Content\Tools\DVWebTool\Application Files\DocsVision.DVWebTool.exe.manifest`. В поле Version будет указана версия, полученная при обновлении файла манифеста.
.. Нажмите menu:File[Save]. Будет предложено подписать файл развёртывания.
.. Нажмите *…* (три точки) в поле _File_, выберите файл сертификата `{wcd-win}/DVWebTool.pfx`, затем нажмите *OK* в основном окне подписания (пароль указывать не нужно).
+
****
...
. Сохраните разметку.
. Установите или обновите программу _DVWebTool_. Cм. пункт xref:webclient:user:prepare-add-components.adoc#dvWebTool[Установка и запуск программы DVWebTool] руководства пользователя {wc}.
