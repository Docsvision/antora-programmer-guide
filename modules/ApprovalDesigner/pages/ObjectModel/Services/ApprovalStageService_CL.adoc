...
a|Вычисляет согласующих при запуске этапа.
...
*Возвращаемое значение*: List<`xref:BackOffice-ObjectModel-Staff:StaffEmployee_CL.adoc[StaffEmployee]`> -- список согласующих
