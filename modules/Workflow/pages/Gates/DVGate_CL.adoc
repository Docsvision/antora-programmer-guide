...
|`xref:Gates/DVGate.CopyCard_MT.adoc[CopyCard(DVCard, Boolean)]` |Создает копию переданной карточки.
|`xref:Gates/DVGate.CopyCards_MT.adoc[CopyCards(Array, Boolean, ProcessVariable)]` |Создает копии нескольких карточек.
|`xref:Gates/DVGate.CopyVariable_MT.adoc[CopyVariable(Int32, String, String)]` |Создает копию переменной шлюза.
...
|`xref:Gates/DVGate.CreateCard_MT.adoc[CreateCard(String)]` |Создает новую карточку указанного типа.
|`CreateCardInFolder(DVCardType, DVFolder)` |Создает новую карточку указанного типа и добавляет ярлык для её открытия в папку.
|`xref:Gates/DVGate.CreateCardInFolder_MT.adoc[CreateCardInFolder(String, DVFolder)]` |Создает новую карточку указанного типа и ярлык на неё.
|`xref:Gates/DVGate.CreateDVFileFromExAttachment_MT.adoc[CreateDVFileFromExAttachment(ExAttachment)]` |Создает карточку версионного файла на основе вложения письма переданного шлюзом к `Exchange`.
|`xref:Gates/DVGate.CreateDVFileFromExAttachmentInFolder_MT.adoc[CreateDVFileFromExAttachmentInFolder(ExAttachment, DVFolder, ProcessVariable)]` |Создает карточку версионного файла на основе вложения письма переданного шлюзом к `Exchange`.
|`xref:Gates/DVGate.CreateDVFileFromFSFile_MT.adoc[CreateDVFileFromFSFile(FSFile)]` |Создает карточку версионного файла на основе файла, полученного из шлюза к файловой системе.
|`xref:Gates/DVGate.CreateDVFileFromFSFileInFolder_MT.adoc[CreateDVFileFromFSFileInFolder(FSFile, DVFolder, ProcessVariable)]` |Создает карточку версионного файла на основе файла, полученного из шлюза к файловой системе, и ярлык на карточку.
|`xref:Gates/DVGate.CreateFolder_MT.adoc[CreateFolder(DVFolder, String, DVCardRow)]` |Создает новую папку указанного типа.
