...
* `Newtonsoft.Json.dll`.
+
. Поместите скопированные файлы в каталог `/usr/lib/docsvision/common` в Linux и `C:\Program Files\Docsvision\Common` в Windows.
...
. Добавьте новое хранилище. В окне настройки хранилища в поле _Тип_ нужно выбрать вариант *_Собственное хранилище_* и укажите имя сборки `GoogleDriveStorageProvider.dll` из каталога на сервере в поле _Имя сборки_. В список типов хранилищ будет добавлено хранилище с названием "Хранилище в Google Drive".
. Настройте новое хранилище с типом "Хранилище в Google Drive", группу и правило помещения в хранилище следуя инструкции, приведенной в разделе "xref:dev@mgmtconsole:servers:db-storage-setup.adoc[]" документации модуля {mc}.
+
При настройке хранилища в блоке _Свойства_ необходимо указать: путь к файлу `credentials.json` и папке `token.json` (загружены в п. 10), путь к файлу журнала работы провайдера.
+
.Пример настройки хранилища в Google Drive
image::ROOT:connector-google-drive.png[Пример настройки хранилища в Google Drive]
+
