...
. Добавьте новое хранилище. В окне настройки хранилища в поле _Тип_ выберите вариант *_Собственное хранилище_* и укажите в поле _Имя сборки_ имя сборки `GoogleDriveStorageProvider.dll` из каталога на сервере, затем нажмите кнопку *Получить данные*.
+
В появившемся блоке _Свойства_ необходимо указать: путь к файлу `credentials.json` и папке `token.json` (загружены в п. 10), путь к файлу журнала работы провайдера.
+
.Пример настройки хранилища в Google Drive
image::ROOT:connector-google-drive.png[Пример настройки хранилища в Google Drive]
+
. В список типов хранилищ будет добавлено хранилище с названием "Хранилище в Google Drive". Настройте для нового хранилища группу и правило, следуя инструкции, приведенной в разделе "xref:dev@mgmtconsole:servers:db-storage-setup.adoc[]" документации модуля {mc}.
