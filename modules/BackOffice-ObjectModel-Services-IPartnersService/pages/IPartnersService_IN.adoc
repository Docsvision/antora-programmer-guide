...
|`GetEmployeesCount(PartnersCompany, Boolean, Boolean)` |Получает количество сотрудников контрагента с учётом иерархии.
...
|`GetUnits(PartnersCompany, Boolean, Boolean)` |Предоставляет коллекцию подразделений, входящих в указанное подразделение, с учётом иерархии.
