...
|`xref:ILockService.IsObjectLockedByAnotherUser_1_MT.adoc[IsObjectLockedByAnotherUser<T>(T, String)]` |Определяет наличие у объекта блокировки, установленной другим сотрудником, а также учётную запись сотрудника, выполнившего блокировку.
