...
|`xref:xref:BackOffice-ObjectModel-BaseCard:BaseCard_CL.adoc[BaseCard]` |Базовый класс для объектно-ориентированных карточек библиотеки _{bo}_.
...
|`xref:xref:BackOffice-ObjectModel-Staff:StaffEmployee_CL.adoc[StaffEmployee]` |Представляет сотрудника подразделения из справочника сотрудников.
