...
|`xref:SectionData.GetAllRows_2_MT.adoc[GetAllRows(Guid, Boolean)]` |Получение всех строк в секции для конкретной строки родительской секции, без учёта уровня иерархии.
