...
. Добавить, используя метод `xref:webservice/rows/RowSetData.adoc[RowSetData]`, в строку секции "Версии" значения полей (см. xref:schemas:standard-card-fields.adoc[Описание полей стандартной карточки]): `FileId` (используем идентификатор файла, полученный в предыдущем примере), `Version` и `VersionNumber`.
